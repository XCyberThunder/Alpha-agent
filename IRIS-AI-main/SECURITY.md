...
If you believe you've found a legitimate security vulnerability in the alpha OS Agent, please report it privately. Do not open public GitHub issues for critical zero-days or remote code execution (RCE) flaws.

## Reporting

Report vulnerabilities directly via email to the Lead Architect:

- **gamerpandeythunder@gmail.com**

Please allow up to 48 hours for an initial response. alpha is currently a solo-developed project, but all severe reports are triaged with the highest priority.
...
5. **Environment** (OS Version, alpha Version)
...
## 🛡️ alpha Trust & Threat Model (CRITICAL)

alpha is **not** a cloud-based web app. It is a local, kernel-level Operating System extension built on Electron. Because of this, the security model operates under the **"Trusted Operator"** paradigm.

### 1. The Trusted Operator Assumption

alpha assumes that anyone who has unlocked the host machine and launched the application is the **Trusted Operator**.

- alpha is designed to execute commands, read files, click the screen, and modify the OS on behalf of the user.
- **If a user explicitly asks alpha to delete a file, and alpha deletes it, that is a feature, not a vulnerability.** * Vulnerabilities are defined as actions taken *without* the user's consent, or malicious escalation *bypassing\* the IPC bridge.

### 2. Single-User Boundary

alpha does **not** model one installation as a multi-tenant, adversarial boundary.

- It is designed for one user per machine/OS profile.
- If multiple mutually untrusted users share the same OS login profile, the security boundary is already broken at the OS level, not by alpha.

### 3. 100% BYOK (Bring Your Own Key) Architecture

Privacy is absolute. alpha operates on a strict zero-trust architecture regarding external servers.
...
- The keys are stored in a local, encrypted `alpha_secure_vault.json` file.
...
The following scenarios are considered expected behavior under the alpha threat model and will be closed as `invalid` or `no-action`:
...
2. **Remote Code Execution (RCE):** Any method where an external, remote attacker can force the alpha engine to execute code without the local Trusted Operator's consent.
...
alpha is a labor of love and an open-source initiative. There is currently no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can secure the engine for the community. The best way to help the project right now is by responsibly disclosing and submitting PRs.
