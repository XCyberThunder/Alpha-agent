...
const glassPanel = 'glass-card liquid-panel rounded-2xl shadow-xl'
...
export default function DashboardView({
  ...
  return (
    <div className="flex-1 p-4 liquid-glass-shell grid grid-cols-12 gap-4 h-full overflow-hidden relative animate-in fade-in zoom-in duration-300 w-full">
      <div
        className={`hidden lg:flex ${
          isLeftCollapsed ? 'col-span-1' : 'col-span-3'
        } flex-col gap-4 h-full z-40 relative overflow-hidden transition-all duration-300`}
      >
        <button
          onClick={() => setIsLeftCollapsed((prev) => !prev)}
          className="absolute top-3 right-3 z-20 glass-button p-1.5 rounded-full text-cyan-300"
          title={isLeftCollapsed ? 'Expand left panel' : 'Collapse left panel'}
        >
          {isLeftCollapsed ? <RiArrowUpSLine size={14} /> : <RiArrowDownSLine size={14} />}
        </button>
        <div className={isLeftCollapsed ? 'invisible pointer-events-none' : ''}>
        <div
        ...
        >
          ...
          {isVideoOn && (
            <button
              ...
              className="glass-button absolute top-2 right-2 z-30 p-1.5 rounded-md text-emerald-300"
            >
        ...
        <div className={`${glassPanel} flex-1 p-4 flex flex-col gap-3`}>
          ...
          <div className="grid grid-cols-2 gap-3 h-full pb-1">
            {systemMetrics.map((m, i) => (
              <div
                key={i}
                className={`glass-card cursor-pointer relative rounded-xl p-3 flex flex-col justify-between overflow-hidden group transition-all duration-300 bg-linear-to-br ${m.bgGradient}`}
              >
                <div
      ...
      <div className="col-span-12 lg:col-span-6 relative flex flex-col items-center justify-center">
        ...
        <div className="absolute bottom-10 z-50">
          <div
          className={`${glassPanel} px-6 py-3 rounded-full flex items-center gap-6 border-cyan-300/20 shadow-[0_0_34px_rgba(34,211,238,0.12)]`}
          >
            <button
              onClick={onVisionClick}
              className={`glass-button cursor-pointer p-3 rounded-full transition-all ${isVideoOn ? 'bg-red-500/20 text-red-400' : 'text-zinc-300'}`}
            >
              {isVideoOn ? <RiSwapBoxLine size={20} /> : <RiCameraLine size={20} />}
            </button>
            <button onClick={toggleSystem} className="relative group mx-2">
              <div
                className={`cursor-pointer p-4 rounded-full border-2 transition-all duration-500 ${isSystemActive ? 'bg-cyan-400 border-cyan-200 text-black shadow-[0_0_24px_rgba(34,211,238,0.65)]' : 'bg-red-500/10 border-red-500/50 text-red-500'}`}
              >
            ...
            <button
              onClick={toggleMic}
              className={`glass-button cursor-pointer p-3 rounded-full transition-all ${isMicMuted ? 'bg-red-500/20 text-red-400' : 'bg-cyan-500/10 text-cyan-300'}`}
            >
