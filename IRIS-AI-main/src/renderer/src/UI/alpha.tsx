...
const Alpha = (props: AlphaProps) => {
  ...
  const handleSendText = async (text: string) => {
    return alphaService.sendTextMessage(text)
  }

  return (
    <div className="h-screen w-full liquid-glass-shell text-zinc-100 font-sans overflow-hidden select-none flex flex-col relative pb-5">
      <div className="h-14 w-full flex items-center justify-between px-6 glass-card rounded-none border-x-0 border-t-0 z-50">
        <div className="hidden lg:flex items-center gap-3">
          <RiShieldFlashLine className="text-cyan-300 text-xl animate-pulse drop-shadow-[0_0_10px_rgba(34,211,238,0.7)]" />
          <div className="flex flex-col leading-none">
            <span className="font-black tracking-[0.2em] text-sm text-zinc-100">
              alpha
            </span>
            <span className="text-[11px] font-mono text-cyan-300/70 tracking-widest">
              NEURAL INTERFACE
            </span>
          </div>
        </div>

        <div className="hidden md:flex gap-2 glass-card p-1 rounded-lg">
          {[
            ...
            <button
              ...
              className={`cursor-pointer px-5 py-1.5 text-[10px] font-bold tracking-widest rounded-md transition-all duration-300 flex items-center gap-2 ${
                activeTab === tab.id
                  ? 'bg-cyan-300/20 text-cyan-200 border border-cyan-300/30 shadow-[0_0_18px_rgba(34,211,238,0.16)]'
        ...
        <div className="flex items-center gap-6 text-[11px] font-mono font-bold opacity-60">
          ...
          <div className="glass-button px-2 py-1 rounded text-zinc-200">
