    ...
    try {
      ...
      if (mode === 'emerald_theme' || mode === 'both') {
        const themeScript = `
          ...
          banner.id = 'alpha-override-banner';
          banner.innerText = '⚠️ NETWORK COMPROMISED // alpha HAS ASSIMILATED THIS DOMAIN ⚠️';
        ...
        const rewriteScript = `
          ...
          function assimilateLogos(selectors) {
            selectors.forEach(selector => {
              ...
              logos.forEach(logo => {
                if (!logo.classList.contains('alpha-hacked')) {
                  logo.classList.add('alpha-hacked');
                  logo.style.visibility = 'hidden'; 
                  
                  const newLogo = document.createElement('div');
                  newLogo.innerHTML = \`<strong style="color:#10b981; font-size:24px; visibility: visible; letter-spacing: 2px; text-shadow: 0 0 10px #10b981;">[ alpha ]</strong>\`;
            ...
            try {
              ...
              if (hostname.includes('youtube.com')) {
                ...
                document.querySelectorAll('yt-formatted-string#video-title').forEach(t => {
                  if (t.innerText && !t.innerText.includes('[alpha]')) {
                    if (Math.random() > 0.5) t.innerText = \`[alpha] \${t.innerText}\`;
                  }
                });
              } 
              ...
              // --- 🟧 AMAZON ---
                ...
                document.querySelectorAll('.a-text-normal, .a-color-base h2').forEach(t => {
                  if (t.innerText && !t.innerText.includes('OVERRIDE')) {
                    if (Math.random() > 0.6) t.innerText = \`[alpha_OVERRIDE]: \${t.innerText}\`;
                  }
                });
              } 
              ...
              // --- 🟪 INSTAGRAM / FACEBOOK ---
                ...
                document.querySelectorAll('span, h1, h2, div[dir="auto"]').forEach(t => {
                  if (t.childElementCount === 0 && t.innerText.length > 10 && !t.innerText.includes('ROOT')) {
                    if (Math.random() > 0.7) t.innerText = \`[alpha_ROOT]: \${t.innerText}\`;
                  }
                });
              }
              ...
              // --- ⬛ GITHUB (FLEX ON DEVS) ---
                ...
                document.querySelectorAll('.repo, .markdown-body p, span.RepoIcon').forEach(t => {
                  if (t.innerText && !t.innerText.includes('HACKED')) {
                    if (Math.random() > 0.6) t.innerText = \`[alpha_HACKED]: \${t.innerText}\`;
                  }
                });
              }
              ...
              // --- 🟦 LINKEDIN (FLEX ON CORPORATES) ---
                ...
                document.querySelectorAll('span[dir="ltr"], .break-words').forEach(t => {
                  if (t.childElementCount === 0 && t.innerText.length > 15 && !t.innerText.includes('SYNDICATE')) {
                    if (Math.random() > 0.7) t.innerText = \`[alpha_SYNDICATE] \${t.innerText}\`;
                  }
                });
              }
              ...
              // --- 🐦 X / TWITTER ---
                ...
                document.querySelectorAll('[data-testid="tweetText"]').forEach(t => {
                  if (t.innerText && !t.innerText.includes('TRANSMISSION')) {
                    if (Math.random() > 0.6) t.innerText = \`[alpha_TRANSMISSION]: \${t.innerText}\`;
                  }
                });
              }
              ...
              // --- 🍿 NETFLIX ---
                ...
                document.querySelectorAll('.slider-item p, .jawBoneContainer h8, .title-card').forEach(t => {
                  if (t.innerText && !t.innerText.includes('STREAM')) {
                    if (Math.random() > 0.5) t.innerText = \`[alpha_STREAM] \${t.innerText}\`;
                  }
                });
              }
              ...
              // --- ⬜ UNIVERSAL FALLBACK ---
                ...
                document.querySelectorAll('h1, h2, h3, p').forEach(t => {
                  if (t.innerText && !t.innerText.includes('alpha')) {
                    if (Math.random() > 0.6) t.innerText = \`[alpha] \${t.innerText}\`;
                  }
                });
              }
