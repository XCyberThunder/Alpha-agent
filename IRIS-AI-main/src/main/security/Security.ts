...
export default function registerSecurityVault() {
  const legacyFace = store.get('alpha_vault_face') as number[] | undefined
  if (legacyFace && !store.get('alpha_vault_faces')) {
    store.set('alpha_vault_faces', [legacyFace])
    store.delete('alpha_vault_face')
  }

  ipcMain.handle('check-vault-status', () => {
    const hasPin = !!store.get('alpha_vault_hash')
    const faces = store.get('alpha_vault_faces') as number[][] | undefined
    const hasFace = faces && faces.length > 0
    return { hasPin, hasFace, faceCount: faces ? faces.length : 0 }
  })

  ipcMain.handle('get-personality', () => {
    return store.get('alpha_personality') as string | undefined
  })

  ipcMain.handle('set-personality', (_, text: string) => {
    store.set('alpha_personality', text)
    return true
  })
  ...
  ipcMain.handle('setup-vault-pin', async (_, pin: string) => {
    ...
    store.set('alpha_vault_hash', hash)
    return true
  })

  ipcMain.handle('verify-vault-pin', async (_, pin: string) => {
    const hash = store.get('alpha_vault_hash') as string
    if (!hash) return false
    return await bcrypt.compare(pin, hash)
  })

  ipcMain.handle('setup-vault-face', (_, descriptor: number[]) => {
    const faces = (store.get('alpha_vault_faces') as number[][]) || []
    faces.push(descriptor)
    store.set('alpha_vault_faces', faces)
    return true
  })
